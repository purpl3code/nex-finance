      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <Button onClick={handlePushToCloud} disabled={isSyncing} className="w-full flex justify-center">
                {isSyncing ? 'Enviando...' : 'Enviar Dados para Nuvem'}
             </Button>
             <Button onClick={handlePullFromCloud} variant="secondary" disabled={isSyncing} className="w-full flex justify-center">
                {isSyncing ? 'Baixando...' : 'Baixar Dados da Nuvem'}
             </Button>
          </div>
          <p className="text-xs text-slate-500 text-center">
            Seus dados são salvos automaticamente na nuvem ao fazer alterações.
          </p>
        </div>
      </SectionCard>
